...
import com.wisesignsoft.OperationManagement.adapter.SingleResModelSelectorAdapter;
...
public class SingleResModelSelectorActivity extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private ResModeValue resModeValue;
    private int currentPage, total;
    private static String extraResKey = "resModeValue", extraIdKey = "woId";
    ...
    public static void start(Context context, ResModeValue resModeValue, String id) {
        Intent starter = new Intent(context, SingleResModelSelectorActivity.class);
        starter.putExtra(extraResKey, resModeValue);
        starter.putExtra(extraIdKey, id);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_single_res_model_selector);
        initView();
        requestModelName();
    }
    ...
    private void initView() {
        resModeValue = (ResModeValue) getIntent().getSerializableExtra(extraResKey);
        final String id = getIntent().getStringExtra(extraIdKey);
        bean = resModeValue.getConfigValueJson();
        modelName = resModeValue.getToBmModelName();
        ...
        /*设置适配器*/
        SingleResModelSelectorAdapter adapter = new SingleResModelSelectorAdapter(this, datas);
        ...
        /*item点击事件*/
        adapter.setOnIContractInfo(new SingleResModelSelectorAdapter.IContractInfo() {
            @Override
...
//                WorkOrderDataManager.getManager().setResModelValueByFromOrTo(bean, datas.get(position));
    ...
    //获取业务主键为TRUE的业务模型识别，根据此key获取值并显示标题位置上
